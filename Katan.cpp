...
Katan::~Katan() {
        for (int i=0; i<players.size(); i++){
            for (int j=0; j<players[i]->getDevelopmentCards().size(); j++){
                delete players[i]->getDevelopmentCards()[j];
            }
        }
...
// Inside the Katan class implementation
Katan& Katan::operator=(const Katan& other) {
    if (this != &other) { // Self-assignment check
        // Copy member variables
        this->players = other.players;
        this->currentPlayerIndex = other.currentPlayerIndex;
        this->board = new Board(*other.board); // Deep copy of board
    }
    return *this;
}
